<!-- PLACEMENT: would land in a new file
     `solidstats-parser-rust-conventions/references/observability-and-lifecycle.md`
     SKILL.md reference map gains one row:
       `references/observability-and-lifecycle.md` | **§K Log hygiene** (tracing structured fields,
       level semantics, state-transition spans), **§L Diagnosability** (swallowed Results, error
       source chains, identifying context, upstream detail), **§M Resource lifecycle** (unbounded
       collections on worker state, bounded channels, temp files, S3 multipart cleanup).
     The existing §H bullet "Instrument with `tracing`…" in worker-build-perf.md becomes a
     forward-reference to §K rather than stand-alone prose. -->

# Observability & resource lifecycle

> Mirrors §Z/§AA/§AB of solidstats-shared-backend-ts-standards (TS form) — keep the two in sync when
> doctrine changes.
