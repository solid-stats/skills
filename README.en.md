...
| Design Review | `solidstats-frontend-react-design-review` | Draft | Pedantic UI / visual / UX review for `web` — the design counterpart to code review and the project overlay for `gsd-ui-review`. Six pillars: tokens & contrast (`design.md lint`), real-width visual at the project breakpoints incl. 1920/2560/ultrawide (Playwright + CLS + back-nav restoration), accessibility (axe / WCAG 2.2 AA), ×5/×4 states, responsiveness, and system + domain adherence. Hard-requires `solidstats-shared-review-standards`; enforces `solidstats-frontend-react-design`. |
