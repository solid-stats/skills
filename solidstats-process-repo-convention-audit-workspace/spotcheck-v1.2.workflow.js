export const meta = {
  ...
  description:
    'Adversarially verify a stratified sample of the v1.2 replays-fetcher@c850190 audit findings (plus the 5 v1.1 🔴 as a regression before/after) against the real source, to measure the v1.2 false-positive rate and test the 🔴 5→0 delta. Parameterized: args.repo + args.sample (array of {ruleId,severity,file,lineStart,lineEnd,quote,citation,kind,lane,mechanical,stratum}) + optional args.model (default "haiku"; pass "opus" for a grading-sensitive re-check) + args.concurrency (default 5 for opus, 12 otherwise — opus is rate-limited).',
  phases: [{ title: 'Verify' }],
}
...
const CONCURRENCY = Number.isInteger(A.concurrency) ? A.concurrency : MODEL === 'opus' ? 5 : 12
...
If any condition fails, \`real\` = false with the reason. Return the schema only.`,
    { schema: VERDICT, label: `check:${x.stratum}:${x.ruleId}:${x.lineStart}`, phase: 'Verify', model: MODEL }
  )
    .then((v) => ({ ...x, verdict: v }))
    .catch(() => null)

// Bounded concurrency: opus is rate-limited (the audit gates it behind a semaphore of 5), so
// process the sample in chunks instead of firing every verifier at once.
const results = []
for (let i = 0; i < SAMPLE.length; i += CONCURRENCY) {
  const part = await parallel(SAMPLE.slice(i, i + CONCURRENCY).map((x) => () => verifyOne(x)))
  results.push(...part)
  log(`verified ${results.length}/${SAMPLE.length} (${MODEL})`)
}
